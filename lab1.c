...
BST* bst_create(CompareFunc cmp, FreeFunc freeKey, FreeFunc freeValue) 
{
...
void _bst_destroy_nodes(Node* node, FreeFunc freeKey, FreeFunc freeValue) 
{
    if (node == NULL) return;
...
Node* _bst_insert(Node* node, void* key, void* value, CompareFunc cmp) 
{
    if (node == NULL) 
    {
...
void bst_insert(BST* tree, void* key, void* value) 
{
    if (tree == NULL) return;
...
Node* _bst_search(Node* node, void* key, CompareFunc cmp) 
{
...
Node* _bst_extract_min(Node* root, Node** min_node) 
{
    if (root->left == NULL)
    {
        *min_node = root;
...
Node* _bst_delete(Node* node, void* key, BST* tree) 
{
    if (node == NULL) return NULL;
...
void _bst_preorder(Node* node, VisitFunc visit) 
{
    if (!node) return;
...
void bst_preorder(BST* tree, VisitFunc visit) 
{
    if (tree) _bst_preorder(tree->root, visit);
}

void _bst_inorder(Node* node, VisitFunc visit) 
{
    if (!node) return;
...
void bst_inorder(BST* tree, VisitFunc visit) 
{
    if (tree) _bst_inorder(tree->root, visit);
}

void _bst_postorder(Node* node, VisitFunc visit) 
{
    if (!node) return;
...
void bst_postorder(BST* tree, VisitFunc visit) 
{
